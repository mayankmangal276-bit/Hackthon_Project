...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    if (isMongo()) {
      ...
      // If this is a shelter, create its operational profile.
      if (role === 'SHELTER') {
        if (
          capacity == null ||
          Number(capacity) <= 0
          ...
          return res.status(400).json({
            message: 'Shelter capacity is required'
          });
        }
        ...
        const shelter = await Shelter.create({
          ...
          foodNeeds: Array.isArray(foodNeeds)
            ? foodNeeds
            : [],

          acceptedFoodTypes:
            Array.isArray(acceptedFoodTypes)
              ? acceptedFoodTypes
              : [],
        ...
        return res.status(201).json({
          ...
          shelter: {
            ...
            foodNeeds: shelter.foodNeeds,
            acceptedFoodTypes: shelter.acceptedFoodTypes,
      ...
      // Normal registration for Donor / Driver / Admin.
      return res.status(201).json({
        message: 'Registered',
